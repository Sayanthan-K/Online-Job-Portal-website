...
<!-- body section-->
		...
		<fieldset class="hdd">
	<p class="hd">INTERVIEWS</p></fieldset>
...
<?php
	...
	$sql="SELECT * 
	FROM interview a,employer b,job c
	WHERE a.employee_id LIKE (SELECT employee_id FROM employee WHERE mail LIKE '$mail') AND a.employer_id LIKE b.company_id AND c.job_id LIKE a.job_id";
	...
	if (mysqli_affected_rows($conn) > 0){
		 ...
		 while ($row = $result->fetch_assoc()){
		 	echo "<fieldset class=\"notif\">
					<p id=\"jobws\">NAME OF COMPANY : <span class=\"det\">".$row['company_name']."</span>
					<br>JOB NAME :  <span class=\"det\">".$row['job_name']."</span>
					<form method=\"post\" action=\"notiPdetails.php\">
					<input type=\"hidden\" value=\"".$row['interview_id']."\" name=\"interview\">
					<input type=\"submit\" class=\"notiV\" value=\"View Details\"></p>
					</form>
				</fieldset>";
		 }
