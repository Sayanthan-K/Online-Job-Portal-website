...
<div class="log">
	<form class="signin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
		<h2 class="sign">Sign In As Employer</h2>
		<input type="mail" name="emailC" placeholder="enter email " class="in" required pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$">
		<span id="msg" style="color: green; padding: 5px 10px;  "></span>
		<input type="password" name="passC" placeholder="enter password" class="in" id="passL" required>
		...
		<input type="submit" class="btn" value="Log In" name="employerlog"> 
		 <p>don't have account <a href="signupC.php" class="linkP">CLICK HERE</a></p>
	...
	if(isset($_POST["employerlog"])){

	$email = $_POST['emailC'];
	$userPASSCHK = $_POST['passC'];
		...
		if ($result = mysqli_query($conn, $sql)) {
		  while ($obj = mysqli_fetch_object($result)) {
		    ...
		    if ($obj->password == $userPASSCHK) {
		    	$_SESSION['mailP'] = $_POST['emailC'];
				$_SESSION['passP'] = $_POST['passC'];
