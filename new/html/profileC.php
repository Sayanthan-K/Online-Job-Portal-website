...
include'config.php';
	...
	$sql = "SELECT * FROM employer WHERE email LIKE '$mail'";
	$result=mysqli_query($conn, $sql);

	if (mysqli_affected_rows($conn) > 0) {
    while ($row = $result->fetch_assoc()){
    	$_SESSION['nameP'] = $name = $row['company_name'];
    	$_SESSION['nameP'] = $row['company_name'];
    	$ceo = $row['ceo_name'];
	...
	$sql = "SELECT * FROM job WHERE company_id LIKE '$id'";
	$result=mysqli_query($conn, $sql);
	$postedjob = mysqli_affected_rows($conn);
	if ($postedjob > 0) {
		$post = "blue";
	}
	...
	$sql = "SELECT * 
	FROM job a, applied_job b 
	WHERE a.company_id LIKE '$id' AND a.job_id LIKE b.job_id ";

	$result = mysqli_query($conn, $sql);
	$application = mysqli_affected_rows($conn);
	if (mysqli_affected_rows($conn) > 0){
		$noti = "blue";
	}
	...
	if(isset($_POST["removephoto"])){
		$sql = "UPDATE employer SET profile_image = 'user1.png' WHERE company_id LIKE '$id'";
	...
	<div class="backL" id="backL">
		<header class="header">
			...
			<ul class="button1">
				...
				<?php if($validation == 2){echo "<li><a href=\"profileC.php\" >PROFILE</a></li>";}?>
...
<!-- body section-->
		...
		<fieldset class="containP">
			<fieldset class="profileE">
				...
				<ul>
					<hr><li>Company Name : <?php echo $name;  ?> <a href="changeuser.php" class="EP">Edit</a></li><hr>
					<li>Name of CEO : <?php echo $ceo;  ?> </li><hr>
					<li>Location : <?php echo $location ; ?> </li><hr>
					<li>Email : <?php echo $mail ; ?> <a href="changemail.php" class="EP">Edit</a></li><hr>
			...
			<fieldset class="profileE">
				...
				<ul>
					<hr><li>No of Posted Jobs : <?php echo $postedjob;  ?></li><hr>
					<li>No of Job Application : <?php echo $application; ?></li><hr>
				</ul>
			</fieldset>
