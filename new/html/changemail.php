...
include'config.php';
	...
	if(isset($_POST["changeemail"])){
		...
		if (mysqli_affected_rows($conn) > 0) {
			$sql="UPDATE employee SET mail = '$nmail' WHERE mail LIKE  '$mail'";
			...
			$sql="UPDATE employer SET email = '$nmail' WHERE email LIKE  '$mail'";
			mysqli_query($conn, $sql);
			if (mysqli_affected_rows($conn) > 0){
			$_SESSION['mailP'] = $nmail;
...
<!-- body section-->
		...
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="Cpass">
			<div id="Cpass">
				<p class="headingwind">Change E-mail</p>
				<input type="mail" id="newmail" name="mail" placeholder="new mail" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" required><br>
				<input type="submit" name="changeemail" id="changeemail" class="editWB">
			</div>
		</form>
