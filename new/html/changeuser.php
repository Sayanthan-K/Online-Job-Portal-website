...
<!-- body section-->
		...
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="Cpass">
			<div id="Cpass">
				<p class="headingwind">Change User Name</p>
				<input type="text" name="Nuser" placeholder="New User Name" required><br>
				<input type="Password" name="passNew" id="passNewR" placeholder="Password" required  >
				...
				<input type="submit" name="changeuser" id="changeuser" class="editWB" >
			</div>
		</form>
	...
	if(isset($_POST["changeuser"])){
		...
		if (mysqli_affected_rows($conn) > 0) {
		  ...
		  while ($obj = mysqli_fetch_object($result)) {
		    ...
		    if ($obj->password == $pass) {
		    	$sql="UPDATE employee SET employee_name = '$user' WHERE mail LIKE  '$mail'";
		    	mysqli_query($conn, $sql);
		...
		$sql="SELECT password FROM employer WHERE mail like '$mail'";

			if ($result = mysqli_query($conn, $sql)) {
		  while ($obj = mysqli_fetch_object($result)) {
		  	
		    if ($obj->pass == $pass) {
		    	$sql="UPDATE employer SET company_name = '$user' WHERE email LIKE  '$mail'";
		    	mysqli_query($conn, $sql);
		    	//include'';
		    }
		    else{
