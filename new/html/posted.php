...
<!-- body section-->
		<div class="cover">
			...
			<span class="buttonP">
				<a href="notiC.php"><i class="material-icons noti" style="font-size:36px; color: <?php echo $noti;?>">notifications</i></a>
				<a href="posted.php"><i class="material-icons fav" style="font-size:36px; color: <?php echo $post;?>">file_upload</i></a>
				<!-- <span class="logout">
				<button><a href="profileC.php">Back to Profile</a></button>
		...
		<fieldset class="hdd">
	<p class="hd">POSTED JOBS</p></fieldset>
<!-- noti section -->
<?php
	...
	$sql="SELECT * 
	FROM job 
	WHERE company_id LIKE (SELECT company_id FROM employer WHERE email LIKE '$mail')";
	...
	if (mysqli_affected_rows($conn) > 0){
		 ...
		 while ($row = $result->fetch_assoc()){
			...
			<p id=\"jobws\">JOB NAME : <span class=\"det\">".$row['job_name']."</span>
			<br>POSTED DATE : <span class=\"det\">".$row['posted_date']."</span>
			<br>DUE DATE : <span class=\"det\">".$row['due_date']."</span>
			</p>
			<form method=\"post\" action=\"..\php\index.php\">
			<input type=\"hidden\" value=\"".$row['job_id']."\" name=\"job\">
			<input type=\"submit\" class=\"notiV\" value=\"Delete Job\" name=\"deletejob\"></p>
			</form>

			<form method=\"post\" action=\"editjob.php\">
			<input type=\"hidden\" value=\"".$row['job_id']."\" name=\"job\">
			<input type=\"submit\" class=\"notiV\" value=\"Edit Job\" name=\"updatejob\"></p>
			</form>
			</fieldset>";
		 }
