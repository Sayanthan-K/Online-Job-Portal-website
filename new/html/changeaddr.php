	...
	if(isset($_POST["changeaddress"])){
		...
		if (mysqli_affected_rows($conn) > 0) {
			$sql="UPDATE employee SET address = '$addr' WHERE mail LIKE  '$mail'";
			...
			$sql="UPDATE employer SET location = '$addr' WHERE email LIKE  '$mail'";
			mysqli_query($conn, $sql);
			if (mysqli_affected_rows($conn) > 0){
...
<!-- body section-->
		...
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" class="Cpass">
			<div id="Cpass">
				<p class="headingwind">Change Address</p>
				<textarea   id="addrchange" placeholder="New Address" required name="addr"></textarea><br>
				<input type="submit" name="changeaddress" id="changeaddress" class="editWB">
			</div>
		</form>
