...
<!-- body section-->
		...
		<fieldset class="hdd">
	<p class="hd">APPLICANTS LIST</p></fieldset>
<!-- noti section -->
<?php
	...
	$sql="SELECT * 
	FROM employee a , job b, applied_job c
	WHERE c.employee_id LIKE a.employee_id AND b.job_id LIKE c.job_id AND b.company_id LIKE (SELECT company_id FROM employer WHERE email LIKE '$mail')";
	...
	if (mysqli_affected_rows($conn) > 0){
		 ...
		 while ($row = $result->fetch_assoc()){
			...
			<br>APPLIED JOB NAME : <span class=\"det\">".$row['job_name']."</span>
			</p>
			<form method=\"post\" action=\"notiCdetails.php\">
			<input type=\"hidden\" value=\"".$row['employee_id']."\" name=\"mail\">
			<input type=\"hidden\" value=\"".$row['job_id']."\" name=\"job\">
			<input type=\"hidden\" value=\"".$row['job_name']."\" name=\"jobname\">
			<input type=\"submit\" class=\"notiV\" value=\"View Applicant Details\"></p>
			</form>
			</fieldset>";
		 }
