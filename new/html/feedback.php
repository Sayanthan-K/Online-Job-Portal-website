...
<div>
	<fieldset class="feed">
		...
		<textarea name="feed" class="feedT" placeholder="feedback.." required></textarea><center>
		<input type="submit" name="feedS" value="submit" class="subF"></center>
	</fieldset>
...
if(isset($_POST["feedS"])){
	...
	$sql = "SELECT company_id FROM employer WHERE email LIKE '$validuser'";
	...
	$sql = "SELECT employee_id FROM employee WHERE mail LIKE '$validuser'";
	if ($result = mysqli_query($conn, $sql)) {
		  while ($obj = mysqli_fetch_object($result)) {
		   $id = $obj->employee_id ;
		}
	...
	if ($validation == 1) {
		$sql = "INSERT INTO feedback (employer_id,message) VALUES ('$id','$feed')";
	...
	if (mysqli_affected_rows($conn) > 0){
		...
		$sql = "INSERT INTO feedback (employee_id,message) VALUES ('$id','$feed')";
	mysqli_query($conn, $sql);
	if (mysqli_affected_rows($conn) > 0){
		echo"<script type=\"text/javascript\">
				alert(\"your feedback is submitted\");
		...
		$sql = "INSERT INTO feedback (message) VALUES ('$feed')";
	mysqli_query($conn, $sql);
	if (mysqli_affected_rows($conn) > 0){
		echo"<script type=\"text/javascript\">
				alert(\"your feedback is submitted\");
