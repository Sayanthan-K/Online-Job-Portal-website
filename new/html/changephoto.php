	<?php
		...
		$photo=$_FILES["fileFieldName"]["name"];
		$sql= "SELECT * FROM employee WHERE mail like '$mail'";
		$result = mysqli_query($conn, $sql);

		if (mysqli_affected_rows($conn) > 0) {
			$sql="UPDATE employee SET profile_image = '$photo' WHERE mail LIKE  '$mail'";
			mysqli_query($conn, $sql);
			

		}
		else{
			$sql="UPDATE employer SET profile_image = '$photo' WHERE email LIKE  '$mail'";
			mysqli_query($conn, $sql);
...
$target_file = $target_dir . basename($_FILES["fileFieldName"]["name"]);
	...
	if (move_uploaded_file($_FILES["fileFieldName"]["tmp_name"],$target_file)){
  		...
  		echo"<script type=\"text/javascript\">alert(\"profile photo uploaded.\");</script>";
	} 
	else {
